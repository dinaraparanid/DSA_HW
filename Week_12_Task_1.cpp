...
int main() {
	...
	dsa_hw::graph::list_graph<int, void*, int> graph(n);
	...
	for (int i = 0; i < n; ++i) {
		for (int q = 0; q < n; ++q) {
			...
			if (w != dsa_hw::NO_EDGE) {
				++edges_num;
				graph.connect_unchecked(i, q, w);
			}
		}
	}

	auto&& negative_cycles_opt = graph.has_negative_cycles();

	if (!negative_cycles_opt.has_value()) {
		std::puts("NO");
		return 0;
	}

	auto&& negative_cycle = negative_cycles_opt.value();
	std::puts("YES");
